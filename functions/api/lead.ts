 ...
 *   - Anti-spam: reject filled honeypot; reject a diagnostic completed < 60s
 ...
 * Progression is never blocked on webhook/email failure — this route returns
 * 200 as long as the request itself is well-formed and not spam.
 */
...
export async function onRequestOptions(ctx: Ctx): Promise<Response> {
  const origin = ctx.request.headers.get('Origin');
  return new Response(null, { status: 204, headers: corsHeaders(ctx.env, origin) });
}
